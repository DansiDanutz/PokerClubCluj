...
export async function POST(req: NextRequest) {
  let body: { email?: string; password?: string; id?: string; visible?: boolean };
  try {
    body = await req.json();
  } catch {
  ...
  if (!email || !password || !id || typeof body.visible !== "boolean") {
    return NextResponse.json({ error: "Date lipsa." }, { status: 400 });
  }

  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/rpc/petition_admin_set_visibility`,
    {
      method: "POST",
      headers: {
